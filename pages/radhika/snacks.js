...
export async function getServerSideProps() {
  ...
  let products = await Product.find({ category: 'snacks', genre: 'radhika' });

  let Snacks = {};

  for (let item of products) {
    if (item.title in Snacks) {
      if (!Snacks[item.title].color.includes(item.color) && item.availableQty > 0) {
        Snacks[item.title].color.push(item.color);
      }
      if (!Snacks[item.title].size.includes(item.size) && item.availableQty > 0) {
        Snacks[item.title].size.push(item.size);
      }
    } else {
      Snacks[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        Snacks[item.title].color = [item.color];
        Snacks[item.title].size = [item.size];
      }
    }
  }

  return {
    props: { products: JSON.parse(JSON.stringify(Snacks)) },
  };
}

...
const Essentials = ({ products }) => {
  ...
  return (
    <section className="text-gray-600 body-font bg-gradient-to-b from-white to-gray-100">
      <Head><title>Essentials</title></Head>
...
export async function getServerSideProps() {
  ...
  let products = await Product.find({ category: 'essentials', genre: 'radhika' });

  let Essentials = {};

  for (let item of products) {
    if (item.title in Essentials) {
      if (!Essentials[item.title].color.includes(item.color) && item.availableQty > 0) {
        Essentials[item.title].color.push(item.color);
      }
      if (!Essentials[item.title].size.includes(item.size) && item.availableQty > 0) {
        Essentials[item.title].size.push(item.size);
      }
    } else {
      Essentials[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        Essentials[item.title].color = [item.color];
        Essentials[item.title].size = [item.size];
      }
    }
  }

  return {
    props: { products: JSON.parse(JSON.stringify(Essentials)) },
  };
}

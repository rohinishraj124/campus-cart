...
const Beverages = ({ products }) => {
  ...
  return (
    <section className="text-gray-600 body-font bg-gradient-to-b from-white to-gray-100">
      <Head><title>Beverages</title></Head>
...
export async function getServerSideProps() {
  ...
  let products = await Product.find({ category: 'beverages', genre: 'radhika' });

  let Beverages = {};

  for (let item of products) {
    if (item.title in Beverages) {
      if (!Beverages[item.title].color.includes(item.color) && item.availableQty > 0) {
        Beverages[item.title].color.push(item.color);
      }
      if (!Beverages[item.title].size.includes(item.size) && item.availableQty > 0) {
        Beverages[item.title].size.push(item.size);
      }
    } else {
      Beverages[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        Beverages[item.title].color = [item.color];
        Beverages[item.title].size = [item.size];
      }
    }
  }

  return {
    props: { products: JSON.parse(JSON.stringify(Beverages)) },
  };
}
